...
class BackLoserBettorTest : AbstractBettorTest<BackLoserBettor>() {
    ...
    fun `place bet - based on best price`() {
        checkPlace(reactorTestFactory.createMarketPrices(2.98, 3.8, 3.0),
                3, 3.75)
    }

    @Test
    fun `place bet - based on fairness`() {
        checkPlace(reactorTestFactory.createMarketPrices(2.98, 3.3, 3.0),
                3, 3.25)
    }

    @Test
    fun `place bet - based on last matched or traded volume`() {
        checkPlace(reactorTestFactory.createMarketPrices(2.98, 3.2, 4.0),
                3, 4.0)
    }

    @Test
    fun `too close price for update`() {
        val market = reactorTestFactory.createMarketPrices(2.8, 3.4, 3.0)
