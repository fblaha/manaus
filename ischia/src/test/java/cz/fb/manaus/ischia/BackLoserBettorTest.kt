...
class BackLoserBettorTest : AbstractBettorTest<BackLoserBettor>() {
    ...
    fun `place bet - based on best price`() {
        checkPlace(persistMarket(reactorTestFactory.createMarket(2.98, 3.8, 3.0, 1)),
                3, 3.75)
    }

    @Test
    fun `place bet - based on fairness`() {
        checkPlace(persistMarket(reactorTestFactory.createMarket(2.98, 3.3, 3.0, 1)),
                3, 3.25)
    }

    @Test
    fun `place bet - based on last matched or traded volume`() {
        checkPlace(persistMarket(reactorTestFactory.createMarket(2.98, 3.2, 4.0, 1)),
                3, 4.0)
    }

    @Test
    fun `too close price for update`() {
        val market = persistMarket(reactorTestFactory.createMarket(2.8, 3.4, 3.0, 1))
