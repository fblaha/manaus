...
class LayLoserBettorTest : AbstractBettorTest<LayLoserBettor>() {
    ...
    fun `place bet - based on fairness`() {
        checkPlace(reactorTestFactory.newMarketPrices(2.98, 3.2, 3.05),
                3, 2.88)
    }

    @Test
    fun `place bet - based on best price`() {
        checkPlace(reactorTestFactory.newMarketPrices(2.58, 3.15, 3.0),
                3, 2.6)
    }

    @Test
    fun `place bet - based on last matched or traded volume`() {
        checkPlace(reactorTestFactory.newMarketPrices(2.8, 3.2, 2.5),
                3, 2.48)
    }
