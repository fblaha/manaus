...
class MarketSnapshotTest {
    ...
    fun `market coverage`() {
        val selectionId = 1L
        val side = Side.LAY
        val predecessor = Bet(null, null, selectionId, Price(2.0, 2.0, side), DateUtils.addHours(Date(), -2), 1.0)
        val successor = Bet(null, null, selectionId, Price(2.0, 2.0, side), Date(), 1.0)
