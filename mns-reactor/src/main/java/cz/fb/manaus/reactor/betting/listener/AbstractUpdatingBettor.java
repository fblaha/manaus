...
public abstract class AbstractUpdatingBettor implements MarketSnapshotListener {
    ...
    public final void onMarketSnapshot(MarketSnapshot snapshot, BetCollector betCollector,
        ...
        var flowFilter = flowFilterRegistry.getFlowFilter(market.getType());
        if (flowFilter.getWinnerCountRange().contains(winnerCount)) {
            var coverage = snapshot.getCoverage();
            var fairness = calculator.getFairness(marketPrices);
            var credibleSide = requireNonNull(fairness.getMoreCredibleSide());
            var ordering = COMPARATORS.get(credibleSide.get());
            var prices = ordering.immutableSortedCopy(marketPrices.getRunnerPrices());
            ...
            for (var i = 0; i < prices.size(); i++) {
                var runnerPrices = prices.get(i);
                var selectionId = runnerPrices.getSelectionId();
                var runner = market.getRunner(selectionId);
                var activeSelection = coverage.contains(side, selectionId)
                        || coverage.contains(side.getOpposite(), selectionId);
                var accepted = flowFilter.getIndexRange().contains(i)
                        && flowFilter.getRunnerPredicate().test(market, runner);
                if (activeSelection || accepted) {
                    var oldBet = ofNullable(coverage.get(side, selectionId));
                    var ctx = contextFactory.create(side, selectionId,
                            snapshot, fairness, accountMoney, categoryBlacklist);
                    setTradedVolumeMean(ctx);
                    var pricelessValidation = validationService.validate(ctx, validators);
                    if (!pricelessValidation.isSuccess()) {
                        cancelBet(oldBet, betCollector);
                        continue;
                    }

                    var newPrice = priceAdviser.getNewPrice(ctx);
                    if (!newPrice.isPresent()) {
                        cancelBet(oldBet, betCollector);
                        continue;
                    }

                    var priceCtx = ctx.withNewPrice(newPrice.get());

                    if (oldBet.isPresent() && oldBet.get().isMatched()) continue;

                    var priceValidation = validationService.validate(priceCtx, validators);
    ...
    private void bet(BetContext ctx, BetCollector betCollector) {
        var action = ctx.createBetAction();
        var newPrice = ctx.getNewPrice().get();

        if (ctx.getOldBet().isPresent()) {
            betCollector.updateBet(new BetCommand(ctx.getOldBet().get().replacePrice(newPrice.getPrice()), action));
        } else {
            var market = ctx.getMarketPrices().getMarket();
            var bet = new Bet(null, market.getId(), ctx.getRunnerPrices().getSelectionId(), newPrice, null, 0);
    ...
    private void setTradedVolumeMean(BetContext context) {
        var tradedVolume = context.getActualTradedVolume();
        if (tradedVolume.isPresent()) {
            var weightedMean = tradedVolume.get().getWeightedMean();
