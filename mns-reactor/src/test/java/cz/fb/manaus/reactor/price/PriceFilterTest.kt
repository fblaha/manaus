...
class PriceFilterTest : AbstractLocalTestCase() {
    ...
    fun `significant prices size`() {
        assertEquals(2, filter.getSignificantPrices(1, samplePrices, bfProvider::matches).size)
        assertEquals(4, filter.getSignificantPrices(2, samplePrices, bfProvider::matches).size)
        assertEquals(6, filter.getSignificantPrices(3, samplePrices, bfProvider::matches).size)
        assertEquals(8, filter.getSignificantPrices(4, samplePrices, bfProvider::matches).size)
        assertEquals(9, filter.getSignificantPrices(5, samplePrices, bfProvider::matches).size)
        assertEquals(10, filter.getSignificantPrices(6, samplePrices, bfProvider::matches).size)
    }

    @Test
    fun `significant prices`() {
        assertEquals(listOf(back1, lay1), filter.getSignificantPrices(1, samplePrices, bfProvider::matches))
        assertEquals(listOf(back1, back2, lay1, lay2),
                filter.getSignificantPrices(2, samplePrices, bfProvider::matches))
        assertEquals(listOf(back1, back2, back3, lay1, lay2, lay3),
                filter.getSignificantPrices(3, samplePrices, bfProvider::matches))
    }
    ...
    fun `best prices`() {
        ...
        for (runnerPrices in market) {
            ...
            val filteredPrices = filter.filter(prices, bfProvider::matches)
